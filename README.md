...
2. **AI 判读方向** — 正则规则先读一遍,Claude 再独立读一遍,两者结论一致且置信度高才给 🟢 信号。AI 只做文本判读、不预测事件本身,给出的方向必须有官方原文的逐字引文支撑;判读规则经 15 个月全部错例的复盘持续加固(当前 v5:时点性快照不给方向、"结算宣告"与"前瞻认定"分层置信、姊妹市场群发文本参数校验);
...
- **来源白名单**:链上"官方澄清"事件任何地址都能发,系统只信任官方发布地址——实测捕获过抢在官方澄清前 4 秒发布的对抗性反向文本,非白名单来源一律隔离,不进判读也不进下单
- **多轮争议红旗**:同一市场发生 ≥2 轮争议重置且官方给出方向时,邮件红旗提示人工核对结算源数字(15 个月唯一一笔无退款损失即此形态)
- **预告家族三闸门**:官方"定时澄清"预告类市场从一刀切禁买改为三闸门制(事件未决拦截 / 入场价防雷 / 来源白名单),依据 2026-07 官方行为规律研究的 15 个月历史重放(三闸门 16 笔全胜 vs 无闸净亏);当前处于 paper 验证期,验证通过后再放实弹
...
| `EXEC_SKIP_FORECAST_TEMPLATE` | `on` 恢复预告模板家族一刀切跳过(默认 `off` = 三闸门制) |
| `EXEC_FORECAST_MIN_PRICE` | 预告家族防雷闸:方向侧入场价下限(默认 0.30) |
| `EXEC_FORECAST_LIVE` | `on` 预告家族实弹执行(默认 `off` = paper 验证期,过闸也不实弹) |
...
2. **AI reads the direction** — a rule-based pass first, then an independent Claude read; only when both agree at high confidence does a signal earn 🟢. The AI judges *text only* — it never predicts the event itself, and every directional verdict must be backed by a verbatim quote from the official text. The reading rules are hardened against every documented 15-month error case (now v5: point-in-time snapshots carry no direction, "settlement declarations" vs "forward-looking determinations" get tiered confidence, sibling-market broadcast texts are parameter-checked);
...
- **Source whitelist**: anyone can emit an on-chain "official context" event — the system only trusts the official publisher addresses. We've caught an adversarial reverse-direction text posted 4 seconds before the real official clarification; non-whitelisted sources are quarantined from both the AI read and execution
- **Multi-dispute red flag**: when a market has been dispute-reset ≥2 times and officials still give a direction, the alert flags "verify the settlement source numbers by hand" (the only unrefunded loss in 15 months had exactly this shape)
- **Forecast-family triple gate**: markets carrying the official "scheduled clarification" template moved from a blanket no-buy to a triple gate (pending-event block / entry-price floor / source whitelist), based on a 15-month replay from the 2026-07 official-behavior study (16/16 wins with the gates vs a net loss without); currently in a paper validation period before live money
...
| `EXEC_SKIP_FORECAST_TEMPLATE` | `on` restores the blanket skip of forecast-template markets (default `off` = triple gate) |
| `EXEC_FORECAST_MIN_PRICE` | Forecast-family price-floor gate: minimum entry price on the directional side (default 0.30) |
| `EXEC_FORECAST_LIVE` | `on` enables live execution for the forecast family (default `off` = paper validation period) |
