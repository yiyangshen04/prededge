  ...
  return (
    <div className="space-y-2.5">
      <div className="flex flex-wrap items-center gap-3">
        {/* Decision filter — segmented control on the status color system */}
        <div className="flex rounded-lg border border-border bg-bg-input p-0.5 gap-0.5">
          {decisions.map((d) => (
            <button
              key={d}
        ...
        <div className="flex items-center gap-1.5">
          <label className="text-[10px] text-text-muted uppercase tracking-[0.1em]">
            Min Yield
          </label>
          <input
            type="number"
            ...
            placeholder="0"
            className="w-16 h-7 bg-bg-input border border-border rounded-md px-2 text-xs font-mono text-text-primary focus:outline-none focus:border-accent-blue/60 transition-colors"
          />
        ...
        <div className="flex items-center gap-1.5">
          ...
          <select
            ...
            className="h-7 bg-bg-input border border-border rounded-md px-2 text-xs text-text-primary focus:outline-none focus:border-accent-blue/60 transition-colors"
          >
            {sortOptions.map((o) => (
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {(filters.tags.length > 0 || filters.excludedTags.length > 0) && (
            <button
              ...
              className="px-2 py-0.5 text-[10px] rounded-md border border-accent-red/30 text-accent-red hover:bg-accent-red/10 transition-colors"
            >
              Clear
            </button>
          )}
          <button
            ...
            className="px-2 py-0.5 text-[10px] rounded-md border border-border text-text-muted hover:text-text-primary hover:border-accent-red/40 transition-colors"
          >
            Deselect All
          </button>
          {displayedTags.map((tag) => {
            ...
            const neutralClass = virtual
              ...
              : "bg-bg-card text-text-secondary border-border hover:border-accent-blue/40 hover:text-text-primary";
            return (
              <button
                key={tag}
                ...
                className={`px-2 py-0.5 text-[10px] rounded-md border transition-colors ${
                  isIncluded
                    ...
                    : isExcluded
                      ? "bg-accent-red/10 text-accent-red border-accent-red/30 line-through opacity-80"
                      : neutralClass
                }`}
              >
                {tag}
          ...
          {otherTags.length > 0 && (
            <button
              ...
              className="px-2 py-0.5 text-[10px] rounded-md border border-dashed border-border text-text-muted hover:text-text-primary hover:border-border-strong transition-colors"
            >
